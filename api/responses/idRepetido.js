 ...
 * Example usage:
 * ```
 *     return res.idRepetido();
 *     // -or-
 *     return res.idRepetido(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function idRepetido(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.idRepetido()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.idRepetido()` called with an Error:', optionalData);
